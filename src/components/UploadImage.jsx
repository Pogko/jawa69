...
function UploadImage() {
  ...
  const uploadImage = async () => {
    ...
    const safeFileName = imageUpload.name.replace(/[^a-zA-Z0-9._-]/g, "_");
    const filePath = `request/${Date.now()}-${uuidv4()}-${safeFileName}`;
    ...
    try {
      ...
      localStorage.setItem(
        "uploadedImagesCount",
        (uploadedImagesCount + 1).toString()
      );
      ...
      Swal.fire({
        icon: "success",
        title: "Success!",
        text: "Your image has been sent for approval.",
        customClass: { container: "sweet-alert-container" },
      });
    } catch (error) {
